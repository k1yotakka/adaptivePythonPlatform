...
- React (версия 18+) в качестве основной библиотеки для построения UI
...
Также есть публичный маршрут вступления по ссылке-приглашению:
- /join/:code — приёмник invite-ссылки. Преподаватель копирует ссылку вида {домен}/join/{код}, и при переходе по ней студент автоматически вступает в группу. Если пользователь не авторизован, код запоминается, и вступление выполняется автоматически после входа или регистрации; авторизованному не-студенту показывается пояснение, что вступать могут только студенты.
...
JWT-токен автоматически добавляется к каждому запросу из localStorage. Базовый URL сервера берётся из переменной окружения VITE_API_URL (на этапе сборки). Если переменная не задана, используется http://localhost:8000/api/v1 — это удобно для локальной разработки. На проде в VITE_API_URL указывается адрес задеплоенного бэкенда, благодаря чему один и тот же код работает и локально, и в развёрнутом виде без правок.
...
1. Редактор кода:
   ...
   - Вставка и перетаскивание текста в редактор отключены (обработчики onPaste и onDrop): это мягкий образовательный барьер, побуждающий студента писать код самостоятельно, а не вставлять готовое решение
